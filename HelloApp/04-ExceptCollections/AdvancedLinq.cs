namespace AdvancedLinq
{
  ...
  class Program
  {
    public static void AdvancedLINQ()
    {
      ...
      var charactersWithAbilities = from c in characters join a in abilities on c.Id equals a.CharacterId select new { c.Alias, c.Name, a.Description };
      ...
      double avengersPower = (from c in characters join s in statistics on c.Id equals s.CharacterId where c.Team == "Avengers" select s.Power).Average();
      WriteLine($"🛡️ Promedio de poder de los Avengers: {avengersPower:F2}");

      var abilitiesByCharacter = from c in characters join a in abilities on c.Id equals a.CharacterId group a by c.Alias into groupAbilities select new { Character = groupAbilities.Key, Count = groupAbilities.Count() };
      WriteLine("📝 Cantidad de habilidades por personaje:");
      foreach (var item in abilitiesByCharacter)
      {
        WriteLine($"{item.Character}: {item.Count}");
      }
    }
  }
}